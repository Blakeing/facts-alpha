<template>
  ...
  >
    ...
    <!-- Empty state -->
    <template v-if="!items.length && !loading" #bottom>
      <div class="d-flex flex-column align-center justify-center pa-8 text-center">
        <v-icon v-if="emptyIcon" :icon="emptyIcon" size="48" color="medium-emphasis" class="mb-4" />
        <p v-if="emptyTitle" class="text-h6 mb-1">{{ emptyTitle }}</p>
        <p v-if="emptySubtitle" class="text-body-2 text-medium-emphasis mb-4">{{ emptySubtitle }}</p>
        <slot name="empty" />
      </div>
    </template>
  </v-data-table>
